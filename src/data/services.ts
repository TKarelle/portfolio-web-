export const process = [
  {
    step: "01",
    title: "On échange 15 minutes",
    text: "On parle vraiment de tes besoins et du tarif. Tu sais ce que ça coûte, ce qui est inclus, et ce qui est possible — sans engagement.",
  },
  {
    step: "02",
    title: "Tu valides l’offre",
    text: "Tu valides l’offre. On pose un cahier des charges clair : ce qu’on livre, le planning, et ce qui est inclus.",
  },
  {
    step: "03",
    title: "Je crée avec un suivi",
    text: "Je construis ton site et je te tiens au courant à chaque étape. Tu vois l’avancement, tu valides, j’ajuste.",
  },
  {
    step: "04",
    title: "C’est en ligne",
    text: "Mise en ligne, petit tour pour modifier l’essentiel. Tu reprends ton métier — le site est là.",
  },
] as const;
