...
export function Footer() {
  return (
    <footer className="bg-bg-dark text-white overflow-hidden rounded-t-[2rem] md:rounded-t-[2.75rem] border-t-[3px] border-x-[3px] border-ink shadow-[0_-6px_0_#ff1f71]">
      <div className="mx-auto max-w-6xl px-5 pt-8 pb-5 md:px-10 md:pt-10 md:pb-6">
        ...
        <div className="mt-7 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 border-t border-white/10 pt-4">
          <p className="text-white/30 text-xs font-medium">
            © {new Date().getFullYear()} {FOUNDER_NAME} — France
          </p>
          <p className="text-white/25 text-xs font-medium">Prix TTC indicatifs</p>
        </div>
      </div>

      <div className="relative px-4 md:px-6 pb-1.5 md:pb-2 overflow-hidden select-none pointer-events-none">
        <p
          className="font-extrabold tracking-tighter leading-[0.82] text-center text-white"
          style={{ fontSize: "clamp(3.75rem, 20vw, 10rem)" }}
          aria-hidden
        >
          {BRAND_LOGO}
